...
export default function ProfilePage() {
  ...
  useEffect(() => { listTransactions(50,1).then(setTx).catch(console.error); }, []);
  ...
  const handleDeposit = async () => {
    const raw = prompt("Amount to DEPOSIT (USD)", "100");
  ...
  const handleWithdraw = async () => {
    const raw = prompt("Amount to WITHDRAW (USD)", "100");
    const amount = Number(raw);
    if (!amount) return;
    ...
    try {
      ...
      if (needOnboarding) {
        try {
          ...
          window.location.href = url;
        } catch { alert("Failed to obtain onboarding link"); }
      } else {
    ...
    <div className="layout">
      ...
      <main className="main-content profile-container">
        ...
        <div className="header-profile">
          ...
          <button className="btn-pill btn-primary" onClick={handleDeposit}>Deposit</button>
          <button className="btn-pill btn-primary" onClick={handleWithdraw}>Withdraw</button>
          <div className="spacer"/>
          <button className="btn-pill btn-danger" onClick={handleLogout}>Logout</button>
        </div>
        ...
        <div className="kpi-inline">
          <span>
            ...
            <b>{payBalance
                  ...
                  : fmtMoney(summary.total_equity)}</b>
          </span>
          <span>
            Total&nbsp;P/L
          ...
          {tx ? (
            <>
              <ul className="tx-list">
                {visibleTx.map(t=>(
                  <li key={t.id}>
                    <span>{dayjs(t.created_at||t.date).format("DD MMM")}</span>
                    <span>{t.type}</span>
