...
function SparklineIcon({ data }) {
  ...
  const series = (data || []).map((p) => ({ date: p.date, gain_percent: p.gain_percent }));
  return (
    <span
      ...
      <main className="main-content">
        ...
        <header className="dash-header">
          ...
          <button
            ...
            onClick={() => { clearCurrent(); window.location.href = '/'; }}
          >
            Logout
          </button>
        </header>
                    ...
                    <ResponsiveContainer width="100%" height={200}>
                      <PieChart>
                        <Tooltip formatter={(v,n)=>[`${(+v).toFixed(1)}%`, n]}
                                 contentStyle={{ background:'#fff', border:'1px solid #E5E7EB', borderRadius:4 }} />
                        <Pie data={alloc} dataKey="share_percent" nameKey="name"
                             innerRadius={60} outerRadius={100} labelLine={false}
                             label={({ name, share_percent }) => `${name} (${share_percent.toFixed(1)}%)`}>
                          {alloc.map((_, i) => <Cell key={i} fill={i%2?'#10B981':'#6366F1'} />)}
                        </Pie>
                      </PieChart>
                    </ResponsiveContainer>
                    <div className="allocation-legend">
                      {alloc.map((e,i)=>(
                        <div key={i} className="legend-item">
                          <span className="legend-dot" style={{ backgroundColor:i%2?'#10B981':'#6366F1' }} />
