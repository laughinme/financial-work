    ...
    const aggCharts = useMemo(() => {
      ...
      invested.forEach((s) => {
        s.balanceEquity.forEach(({ date, balance, equity }) => {
          const cur = mapBE.get(date) || { balance: 0, equity: 0 };
          mapBE.set(date, { balance: cur.balance + balance, equity: cur.equity + equity });
        });
        s.dailyPL.forEach(({ date, pl }) =>
          mapPL.set(date, (mapPL.get(date) || 0) + pl)
        );
      });
  
      return {
        balanceEquity: Array.from(mapBE, ([date, v]) => ({ date, ...v })),
        dailyPL:       Array.from(mapPL, ([date, pl]) => ({ date, pl })),
      };
    }, [invested]);
