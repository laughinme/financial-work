...
async def myfx_job():
    async with get_uow_manually() as uow, httpx.AsyncClient(timeout=15) as client:
        ...
        if new_oids:
            portfolios += await p_repo.bulk_insert_from_accounts([accounts[oid] for oid in new_oids])
        ...
        for p in portfolios:
            ...
            if p.units_total == 0: nav_price = Decimal('1') 
            else: nav_price = (acc.equity / p.units_total).quantize(Decimal('0.00000001'))
        ...
        logger.info(
            "MyFX sync completed: %s updates / %s snapshots / %s gains",
            len(update_rows), len(snapshot_rows), len(gain_rows)
        )
